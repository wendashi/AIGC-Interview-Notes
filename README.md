...
- 针对 active voxel (occupancy)和 voxel 中的代表点(vertices)，用 V2M LUT(Voxel to Mesh LookUp Table)来建面:
 
  1. 整体流程: NUDF/O,V -> thinning -> LUT 建面 -> post process.
     - 输入: active voxel (occupancy) + dual vertices (voxel 中的代表点)
        ...
        2. 基于每个 expanded cube 中所有 voxels 的贡献，得到的 occupancy mask。用该 mask 查询 V2M LUT (Total 17 categories)；
          ...
          5. Group2 -> 歧义 case 同时检查 6 个面邻居，确定局部建面规则.
             - Group2.1 (5 categories) Anchor-Free: 当前 expanded cube 单独看有歧义；加入相邻 expanded cube 的 occupancy context 后，建面方式即可唯一确定，而且不需要额外 anchor。
             - Group2.2 (3 categories) Anchor Group：邻居判断后，已有 dual vertices 仍不足以形成正确的分支/封闭连接，因此需增加 anchor vertex，作为三角面或 junction 的连接中心。
...
- 尾数位：决定小数有多精细，影响计算误差。
  ...
  |---|---:|---:|---:|---:|---|
  | FP32 | 1/8/23 | 最大 | 约 7 位十进制有效数字 | 4 Bytes | 高稳定训练、敏感计算 |
  | FP16 | 1/5/10 | 很小 | 约 3 位 | 2 Bytes | 混合精度训练、推理 |
  | BF16 | 1/8/7 | 接近 FP32 | 约 2～3 位 | 2 Bytes | 大模型训练首选 |

- FP16, BF16, FP32 的切换场景:
  | 问题 | 建议切换 | 原因 |
  |---|---|---|
  | FP16 梯度下溢，变成 0 | **FP16 → BF16** | BF16 指数范围更大，能表示更小的数 |
  | FP16 梯度溢出，出现 Inf/NaN | **FP16 → BF16** | BF16 数值上限接近 FP32，更难溢出 |
  | BF16 仍出现 Inf/NaN | **BF16 → FP32 排查** | 判断是精度问题，还是学习率、损失函数等真正失稳 |
  | 小数误差、数值抵消明显 | **BF16 → FP32** | BF16 尾数较短，小数精度最低 |
  | 显存不足、训练太慢 | **FP32 → BF16** | 显存约减半，通常比 FP16 更稳定 |

- FP16 和 BF16 都是 16 bit，显存占用基本相同。
- BF16 指数范围更大，不容易梯度溢出；代价是尾数精度略低。
